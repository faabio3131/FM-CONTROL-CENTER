# ADR-011 — Fila/event infrastructure
...
Não introduzir fila/event bus na F06/F07 por padrão. Começar com requests/webhooks e processamento controlado. Adotar queue somente quando connector, latência, volume, retry assíncrono ou desacoplamento operacional fornecerem requisito concreto.

## Justificativa

O System Design permite filas, mas o Current não comprova necessidade que justifique custo/complexidade.

## Consequências

Evita overengineering. O risco é precisar adicionar fila quando integrações crescerem.

## Riscos e mitigação

Connector contract deve manter idempotência/correlation de forma que uma fila futura possa ser adicionada sem alterar semântica.
